...
<ul class="layim-msgbox" id="LAY_view">
    @foreach($list as $k=>$v)
        @if($v->type == 0)
            <li data-uid="{{ $v->fromUser->id }}" data-fromgroup="{{ $v->group_id }}">
                <a href="javascript:;">
                    <img style="width: 40px;height: 40px" src="{{ $v->fromUser->avatar }}"
                         class="layui-circle layim-msgbox-avatar"></a>
                <p class="layim-msgbox-user">
                    <a href="javascript:;">{{ $v->fromUser->nickname }}</a>
                    <span>{{ $v->time }}</span></p>
                ...
                <p class="layim-msgbox-btn">
                    @if($v->status == 0)
                        <button class="layui-btn layui-btn-small"
                                onclick="agree({{ $v->id }},$(this),'{{ $v->fromUser->avatar }}','{{ $v->fromUser->nickname }}')">
                            同意
                        </button>
                        <button class="layui-btn layui-btn-small layui-btn-primary"
                                onclick="refuse({{ $v->id }},$(this))">拒绝
                        </button>
                    @else
        ...
        @else
            <li class="layim-msgbox-system">
                <p>
                    <em>系统：</em>{{ $v->fromUser->nickname }} 已经{{ $v->status == 1 ? '同意' : '拒绝' }}你的好友申请
                    <span>{{ $v->time }}</span></p>
            </li>
        @endif
    @endforeach
